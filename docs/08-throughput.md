...
### Звідки взялася форма розподілу

Детальний розбір violin-діаграми (Графік 4) — як її читати і що розкриває форма понад avg/max — наведено в [розділі 07 (Фаза 6. Візуалізація)](07-visualizations.md). Тут важливий лише її головний підсумок для пропускної здатності: перевага Priority для критичних — це **щільний, передбачуваний** розподіл (а не просто нижче середнє), а ціна для легких — **товстий хвіст** голодування (а не один максимум).
