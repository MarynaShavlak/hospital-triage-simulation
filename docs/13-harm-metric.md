...
Графік показує % критичних у небезпеці за різних порогів (5–30 хв; повна таблиця з колонкою "Розрив" — нижче, у розборі графіка).
...
Для кожного рівня тяжкості — % у небезпеці під трьома дисциплінами (числа — у таблиці "Повна картина за рівнями" вище). Зверніть увагу на **патерн, що розвертається**:
...
2. **Priority втричі знижує ризик для критичних** (68% → 23%), але ціною небезпеки для легких (голодування виштовхує 33% L5 за поріг).
...
**Ключова теза:** замість "у 5 разів швидше" сильніше звучить **"під звичайною чергою 2 з 3 критичних пацієнтів чекають небезпечно довго; пріоритетна черга знижує це втричі"**. Хвилини абстрактні — життя під загрозою ні.
