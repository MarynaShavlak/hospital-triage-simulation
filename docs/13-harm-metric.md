...
- **FIFO** небезпечний для **тяжких** (L1: 68%, L2: 59%), зате найлегших (L5) майже не зачіпає (4%).
- **Priority** захищає тяжких (L1: 23%), але створює небезпеку для **легких** (L5: 33% — через голодування!).
- **Aging** — **найрівніший профіль**: ніде немає катастрофи. Тяжких захищає майже як Priority (L1: 28%), а легких рятує краще за Priority (L5: 26% проти 33%).
